...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "eegnet","AllData_PseudoTrainAndTest")
OUTPUT_DIR = os.path.join(BASE_DIR, "results", "eegnet", "AllData_PseudoTrainAndTest")
...
def extract_subject_id(filename):
    # assumes CONDITION_subjectID.npy
...
def clean_sub_id(k):
        ...
        for part in parts:
            if "sub-" in part:
                return part
        return parts[-1]  # Fallback to last block if no explicit tag
...
def main():
    ...
    # Custom Legend Elements explaining standard box anatomy explicitly
    variability_legends = [
    ...
    ax.set_ylabel("Classification Accuracy (%)")
    ax.set_xlabel("")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{SUFFIX}_RunVariability.pdf"), bbox_inches="tight")
    plt.close()
    ...
    plt.xlabel("")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{SUFFIX}_PerClassAccuracy.pdf"), bbox_inches="tight")
    plt.close()
    ...
    save_latex_table(
        df_stats,
        f"{SUFFIX}_MainStats.tex",
        f"EEGNet {STRATEGY_NAME} classification performance.",
        f"tab:{SUFFIX}_main_stats"
    )
    ...
    for cond in CONDITIONS:
        ...
        for sub in subject_keys(results[cond]):
            per_class_all.append(
                results[cond][sub]["per_class_accuracy"] * 100
            )
        ...
        class_rows.append({
            "Condition": cond,
            "Visual": per_class_mean[0],
            "Spatial": per_class_mean[1],
            "Verbal": per_class_mean[2]
        })
    ...
    save_latex_table(
        df_class_table,
        f"{SUFFIX}_PerClassStats.tex",
        f"Per-class decoding accuracy for EEGNet {STRATEGY_NAME}.",
        f"tab:{SUFFIX}_perclass"
    )
    ...
    subjects = sorted(list(set([
        clean_sub_id(k)
        for k in subject_keys(results[CONDITIONS[0]])
    ])))
    ...
    for sub in subjects:
        ...
        for cond in CONDITIONS:
            ...
            # Protected lookup loop with default fallback parameter
            matching_key = next(
                (k for k in cond_dict.keys() if clean_sub_id(k) == sub),
                None
            )

            if matching_key is not None:
                row[cond] = cond_dict[matching_key]
            else:
                row[cond] = np.nan  # Fallback gracefully instead of throwing StopIteration
    ...
    save_latex_table(
        df_subjects,
        f"{SUFFIX}_SubjectAccuracies.tex",
        f"Subject-wise decoding accuracies for EEGNet {STRATEGY_NAME}.",
        f"tab:{SUFFIX}_subjects"
    )
