...
# Settings
CHANCE = 33.33
CONDITIONS = ["BSL", "SENSORY", "DELAY"] #["BSL", "SENSORY", "DELAY"] 
...
def save_stats_table(rows, results_dir, filename, caption, label):
...
def main(results_dir, suffix):
    ...
    print(f"Processing EEGNet {strategy_name} matrix charts...")
    
    # Ensure the designated output folder exists before executing figures
    ...
    for cond in CONDITIONS:
        ...
        for name_a, suffix_a, name_b, suffix_b in comparisons:
            ...
            try:
                res_a = load_results(os.path.basename(results_dir), suffix_a, cond)
                res_b = load_results(os.path.basename(results_dir), suffix_b, cond)
                ...
                stats_rows.append({
                    ...
                    "Method A": name_a,
                    "Method B": name_b,
                    "Mean A (%)": np.mean(vec_a),
                    "Mean B (%)": np.mean(vec_b),
                    "t-value": t,
                    "p-value": p
                })
    ...
    for cond in CONDITIONS:
        ...
        try:
            ...
            stats_rows.append({
                "Comparison": "Pseudo vs NoPseudo",
                "Condition": cond,
                "Mean Accuracy (%)": np.mean(pseudo_vec),
                "t-value": t,
                "p-value": p
            })
    ...
    if suffix == "tl":
        save_stats_table(
            stats_rows,
            results_dir,
            f"{suffix}_StatisticalTests.tex",
            f"Statistical tests for EEGNet {strategy_name}.",
            f"tab:{suffix}_stats"
        )
    ...
    for cond in CONDITIONS:
        ...
        table_rows.append({
            "Condition": cond, 
            "Mean Acc +- SD (%)": f"{mean_acc:.2f} ± {std_acc:.2f}",
            "95% CI": ci95,
            "t-value": t, 
            "p-value": p
        })
...
if __name__ == "__main__":
    if not os.path.exists(RESULTS_BASE):
        ...
        experiments = [d for d in os.listdir(RESULTS_BASE) if os.path.isdir(os.path.join(RESULTS_BASE, d))]
        suffixes = ["sd", "si", "tl"]

        for exp in experiments:
            exp_dir = os.path.join(RESULTS_BASE, exp)
            for suffix in suffixes:
                sample_file = os.path.join(exp_dir, f"BSL_{suffix}.npy")
                if not os.path.exists(sample_file):
                    continue
                print(f"Running automated analysis pipeline for: {exp} [{suffix.upper()}]")
                main(exp_dir, suffix)